...
public class TokenServiceImpl implements TokenService{
    ...
    //按照指定格式生成token，token客户端标识-USERCODE-USERID-CREATIONDATE-RANDEM[6位]
    @Override
    public String generateToken(String userAgent, ItripUser user) {
        StringBuilder str = new StringBuilder("token:");
        //1.User Agent 中文名为用户代理，简称UA，他是一个特殊的字符串头。注意：在浏览器中按F12->Headers可以看到User-Agent
        //   java中通过浏览器请求头（User-Agent）获取浏览器类型，操作系统类型，手机机型
        //2.UsrAgent agent = UserAgent.parseUserAgentString(userAgent); 得到用户代理对象
        //3.Brwser browse = userAgent.getBrowser();获取浏览器
        //4.OperatingSystem os = UserAgent.getOperationgSystem();  获取操作系统
    ...
    public void save(String token, ItripUser user) {
        if(token.startsWith("token:PC-")){  //PC端，有效期是 秒 为单位
    ...
    public boolean validate(String userAgent, String token) {
        ...
        //按“-”进行分割后取第四位，即得到使用MD5对http请求中的user-agent加密后的六位随机数
