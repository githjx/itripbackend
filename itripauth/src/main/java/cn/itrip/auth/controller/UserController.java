...
public class UserController {
    ...
    @ApiOperation(value = "使用邮箱注册", httpMethod = "POST", protocols = "HTTP", response = Dto.class, notes = "使用邮箱注册")
    @RequestMapping(value = "/doregister", method = RequestMethod.POST)
    @ResponseBody
    public Dto doRegister(@RequestBody ItripUserVO userVO) {
        if (!validEmail(userVO.getUserCode())) {
            return DtoUtil.returnFail("请使用正确的邮箱格式", ErrorCode.AUTH_ILLEGAL_USERCODE);
        }
        try {
    ...
    @ApiOperation(value = "唯一验证", httpMethod = "GET",
            protocols = "HTTP", response = Dto.class, notes = "邮箱注册验证")
    @RequestMapping(value = "/ckusr",method = RequestMethod.GET)
    @ResponseBody
    public Dto ckusr(@ApiParam(name = "name", value = "邮箱号码") @RequestParam String name) {
        try {
            ItripUser itripUser = userService.findByUsername(name);

            if (itripUser == null) {
                return DtoUtil.returnSuccess("用户可用");
            } else {

                return DtoUtil.returnFail("用户不可用",ErrorCode.AUTH_USER_ALREADY_EXISTS);
            }
        } catch (Exception e) {
            e.printStackTrace();
