...
public class LoginController {
    ...
    public Dto login(@RequestParam String name, @RequestParam String password, HttpServletRequest request){
        try {
            ...
            if (EmptyUtils.isNotEmpty(user)) { //如果对象不为空
                //得到请求头中的User-Agent
                String userAgent = request.getHeader("user-agent");
                //生成token字符串
                String token = tokenService.generateToken(userAgent,user);
                tokenService.save(token,user);//1、将Token保存至Redis缓存数据库
                //2、返回生成好的token信息。在ItripTOkenVo中就保存了Token对象的所有信息
                //下面构造方法中，参数一Token对象，参数二有效时间为两小时，参数三生成时间
                ...
                return DtoUtil.returnFail("用户密码不正确，请确认后再输入！", ErrorCode.AUTH_ACTIVATE_FAILED);
            }
        }catch (Exception e){
